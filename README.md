...
3. Setup reads the project's real work item types (Epic/Feature/User Story/Bug/Task, or whatever your process defines) and their statuses from Azure DevOps — only types that actually have work items in the project, so unused process defaults (Impediment, Risk, Test Case, ...) never show up. It also discovers every team in the project and each one's card field settings (Parent/AssignedTo visibility). Setup then asks whether to generate placeholder skill files automatically for each individual status. This creates `.kanbrain/config.json` (commit it — it's shared team config) and, if you said yes, one skill file per work item type + status under `.kanbrain/skills/`, registered in `skills` and wired to each status via `workflowSteps` (see below).
4. Edit the generated skill files and, if needed, `.kanbrain/config.json`'s `skills` registry and `workflowSteps` map to fine-tune which skill runs for which status.

   Kanbrain keeps skills in two places. `.kanbrain/config.json`'s `skills` map (`{ [id]: skillEntry }`) is the flat registry: one entry per skill file, each with a required `path` (relative to the workspace root) and optional `label` (overrides the button text — defaults to the skill file's name), `textColor`/`buttonColor` (hex, no `#` needed — override the button's text/background color; an invalid or missing value falls back to the VS Code theme's default button colors), and `isGlobal` (see below). `workflowSteps` (`{ [workItemType]: { [status]: stepOrNull } }`) then links each status to a skill by id — a step is either `null` (no action for that status) or an object with a `skillId` plus optional `definitionOfDone`/`artifacts` string arrays (a checklist and a list of expected deliverables for that step, both shown to the agent when the skill runs). `Kanbrain: Setup` generates registry entries with `label`/`textColor`/`buttonColor` already filled in: `label` is `"Execute {status} skill"`, `buttonColor` is the status's real Azure DevOps color (a neutral gray if none is known), and `textColor` is computed for readable contrast against it — edit any of them by hand (or via the color pickers on the Skills tab) to customize further. `Kanbrain: Sync Board Configuration` only ever adds new workflow steps pointing at bare, unstyled entries, since it's meant to be a light touch that never surprises you with visual changes to entries it didn't create.
   ...
   Any skill in the registry can be flagged `isGlobal: true` — a **global skill** isn't tied to any status, and shows in a small "▾" menu next to the status skill button on the active work item's card (or alone, if the current status has none), so it can run against whatever work item is active regardless of its status. Setup seeds one, `explain-card`, which asks the agent to explain the active work item in plain language. Toggle the "Global" checkbox on a skill's row on the Skills tab to add more of your own, or edit `.kanbrain/config.json`'s `skills` entries directly. Setup (and Sync, if it's missing) also writes `.kanbrain/USAGE.md` — a single reference guide, meant to be read by your coding agent as well as your team, covering the skill registry, workflow steps, and the fact that Kanbrain runs with the same Azure DevOps access your VS Code session already has.
...
- [ ] `Kanbrain: Setup`, after picking a project, asks whether to generate placeholder skill files per work item type/status, and writes a `skills` map reflecting the project's real process either way.
- [ ] Answering "Yes" creates one skill file per work item type + status under `.kanbrain/skills/`, and `Done`/`Removed`-category statuses map to `null`.
...
- [ ] The search dialog shows an "All" tab plus one tab per work item type, in the order the types appear in `.kanbrain/config.json`'s `skills` map; clicking a tab filters the already-visible list instantly, with no loading delay.
- [ ] Each work item type tab's count reflects the total number of that type of work item in the whole project (not just how many match the current search text), and only changes when the dialog is reopened or cleared — not while typing.
- [ ] A work item type tab with 0 items in the project stays visible (dimmed), and clicking it shows the "No work items found." message.
...
- [ ] With no active work item, the panel shows the Home screen (Flow / Team (if any team is discovered) / Commands / Configuration sections, in that order) instead of a bare search box; the Flow section shows a single "Select Work Item" button rather than an embedded list.
...
- [ ] With an active work item, clicking "🏠 Home" shows the Home screen with that item's card in the Flow section (no skill action button on it there), with small 🔍/✕ icon buttons in the card's top-right corner for Switch/Clear, without clearing the active work item; clicking "➡️ Open Flow" returns to the Flow screen (which does show the skill action button).
...
- [ ] That generated file lists the project's real work item types and statuses (with categories), and, separately, the team's real boards and columns (with each column's state mappings by work item type) — or a "No boards were found" note if the team has none.
...
- [ ] `.kanbrain/config.json`'s `cardSettingsByTeam` entries map boards to work item types to `{ parent, assignedTo }` matching each team's real Board Settings > Fields (compare against a couple of types in the real Azure DevOps project — Parent shows as an additional field, AssignedTo as the "Show assigned to" core field).
- [ ] The main Flow card shows a "Parent" label followed by the parent's type icon and "#id: Title" as a clickable link (styled like a native Azure Boards card field) only for a work item type whose (selected/default) team has Parent enabled; a type with Parent disabled (or with no `parent` resolved) shows no such row, and clicking it opens that parent's detail tab without changing the active work item.
- [ ] The assignee row on the main card, subtask cards, and the Home screen's card follows `cardSettingsByTeam`'s `assignedTo` per type (resolved via the selected/default team) — not the "Show assignee in search results" checkbox, which only affects the search dialog's results list.
...
- [ ] The Development section is never gated by any Config screen setting — it shows the same way regardless of `showAssignedTo`, `cardSettingsByTeam`, or the selected team.
...
- [ ] `Kanbrain: Setup` writes `.kanbrain/skills/explain-card.md` and a `skills.explain-card` entry (with `isGlobal: true`) in `.kanbrain/config.json`.
