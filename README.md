...
- [ ] The main Flow card shows a "Parent" label followed by the parent's type icon and title as a clickable link (styled like a native Azure Boards card field, no `#id` shown) only for a work item type whose board has Parent enabled; a type with Parent disabled (or with no `parent` resolved) shows no such row, and clicking it opens that parent's detail tab without changing the active work item.
