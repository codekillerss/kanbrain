...
5. Once confirmed, update every entry's \`label\` in \`.kanbrain/config.json\`'s \`backlogLevels\` map to the agreed real step name — not the auto-generated \`"{backlog level} — {category}"\` placeholder Setup fills in by default.
...
7. Once the final mapping is settled, delete any file under \`.kanbrain/skills/\` that no longer has a \`backlogLevels\` entry pointing at it — don't leave unused skill files behind.
