...
Kanbrain shows the active Azure DevOps work item in a VS Code side panel, with per-status "skill" buttons. Each button generates a context file — this file was generated the exact same way — and sends a "read this file" command to an agent running in an integrated terminal. That agent is you. \`.kanbrain/config.json\` keeps skills in two places: a flat registry (\`skills\`, one entry per skill file with its own \`label\`/colors) and a per-(work item type, status) map (\`workflowSteps\`) that points each status at a \`skillId\` from that registry. The result we're aiming for is one skill for each real step of the team's flow — not necessarily one per raw status name.

## Important nuance: status vs. board column

Kanbrain only understands **status** (\`System.State\`) per work item type — \`workflowSteps\` maps exactly **one skill per status, per work item type**. There is no board-column mode to choose between; board columns aren't a real Kanbrain configuration option, they're listed below purely for your reference. Many teams still think and work in terms of **board columns** rather than raw statuses (common, and often the more natural mental model) — a column can group several statuses together, or have a name that doesn't match any status. When that's the case here, the way to honor it is to point every status that belongs to the same column at the *same* \`skillId\` — not to look for a column-level setting that doesn't exist.

## The skill registry

Every skill, whether wired into a status or not, lives in \`.kanbrain/config.json\`'s \`skills\` registry. A skill not wired into any \`workflowSteps\` entry shows up as a small "▾" menu next to the status skill button on the active work item's card, and runs against whatever work item is active regardless of its status. \`Kanbrain: Setup\`/\`Kanbrain: Sync Board Configuration\` already seed one such skill, \`explain-card\`, that explains the active work item in plain language. See \`.kanbrain/USAGE.md\` for the full guide — including why you (the agent) already have real access to this project's Azure DevOps board data, and can suggest board actions to the user when it makes sense.
...
5. Once confirmed, update each skill's \`label\` in \`.kanbrain/config.json\`'s \`skills\` registry to the agreed real step name — not the auto-generated \`"Execute {status} skill"\` placeholder Setup fills in by default. A skill shared by several statuses only needs its label set once.
6. For every skill file that stays in use, write real, useful instructions for that flow step into it — not a placeholder. Every generated context file already starts with a card info block (id, title, type, status, description, parent, subtasks), so skill files don't need to restate that — just write the instructions. If an instruction needs to reference a specific field inline, skill files can still use \`{{id}}\`, \`{{title}}\`, \`{{description}}\`, \`{{status}}\`, \`{{type}}\`, \`{{url}}\`, \`{{branch}}\`, \`{{parent.id}}\`, \`{{parent.title}}\`, \`{{parent.description}}\`, and \`{{subtasks}}\` placeholders, resolved with the real work item's data every time a skill button runs.
7. Once the final mapping is settled, delete any file under \`.kanbrain/skills/\` that no longer has a \`skills\` registry entry pointing at it — don't leave unused skill files behind.
8. Only touch the real Azure DevOps board (moving statuses between columns, renaming columns, etc.) if the user explicitly asks for that, and only using your own tools/credentials. The one thing Kanbrain writes on its own is the work item's status, when the user changes it from a Flow card; everything else is still yours to do.
`;
}
