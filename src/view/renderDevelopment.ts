...
function renderDevelopmentItem(
  ...
  if (link.kind === 'branch') {
    ...
    const branchTagHtml = renderBranchTag(link.branchName, isMapped ? [link.repositoryId, link.branchName] : null);
    return `<div class="kb-dev-row">${repoTagHtml}${branchTagHtml}</div>`;
  }
  ...
  const prLinkHtml = `<a class="kb-dev-item" href="command:kanbrain.openPullRequestDetail?${commandArgs}" title="${label}">${PULL_REQUEST_ICON}<span class="kb-dev-item-text">${label}</span></a>`;
  return `<div class="kb-dev-row">${repoTagHtml}${prLinkHtml}</div>`;
}
