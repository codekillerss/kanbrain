...
- Split skill configuration into two concerns: a flat **skill registry** (`.kanbrain/config.json`'s `skills`, one entry per skill file with its own path/label/colors and an `isGlobal` flag), and a separate **Workflow** tab that maps each work item type/status to a skill from that registry plus an optional Definition of Done and expected artifacts list — both injected into the generated context file whenever that status's skill runs. Existing configs are migrated automatically the first time they're read. The Skills tab now only manages the registry (with a "Global" checkbox controlling whether a skill shows in the card's "▾" menu regardless of status), and the Workflow tab has its own **Configure with AI** button, which checks whether skills already exist before proposing a status → skill mapping.
