...
- The Flow screen's main card now shows the work item's status as a dropdown, and picking a value writes it straight to Azure DevOps. The options are the statuses that type really has on the board — the ones discovered at Setup — so nothing new is fetched to show the list. The write is deliberately narrow: it is the only field the panel changes, it happens only from the main card (children, the Home card and search results still show the status as plain text), and there is no confirmation dialog, because choosing from a dropdown is already an explicit act. While the write is in flight the 5-second poll pauses, so a response already in transit cannot revert your choice under the cursor, and once it settles the panel re-reads the work item rather than trusting the value it just sent — a process rule may change other fields alongside the state. If Azure DevOps refuses the transition, its own message appears in a VS Code notification and the dropdown snaps back to the board's real value, instead of failing quietly the way a failed poll does.
